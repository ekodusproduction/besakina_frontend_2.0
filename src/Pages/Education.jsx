...
const Education = () => {
  ...
  useEffect(() => {
    fetcheducationAds();
    fetcheducationBanner();
  }, [page]);

  const fetcheducationAds = () => {
    setLoading(true);
    axiosInstance
      .get(`api/education/list?limit=12&page=${page}`)
      .then((response) => {
        const data = response.data.data.education;
        if (data.length > 0) {
          setEducationList((prevData) => [...prevData, ...data]);
        } else {
          setHasMore(false);
        }
        setLoading(false);
  ...
  const fetcheducationBanner = () => {
    setLoading(true);
    axiosInstance
      .get(`api/banner?type=Education`)
      .then((response) => {
        ...
        <div className="py-4 flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <div className="w-full max-w-[300px] bg-white">
              <div
                className="bg-[#3484A1] px-2 py-[3px] rounded shadow text-white flex flex-row items-center gap-2 cursor-pointer w-fit"
