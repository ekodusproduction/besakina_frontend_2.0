...
const AddProfile = () => {
    ...
    useEffect(()=>{
        axiosInstance.get('api/users/details',{
            headers:{
                Authorization:`Bearer ${token}`
            }
        })
    ...
    <form className='w-full bg-white rounded shadow p-6' onSubmit={profileSubmitHandler}>
            ...
            <div>
                    ...
                    {(profilePic || userDetails?.profile_pic?.length) ? (
                        ...
                        <input id='profile' type='file' accept='image/*' name='profile_pic' className='hidden' onChange={handleProfilePicChange} />
                    </div>
                </div>
                ...
                <div className='flex flex-col gap-2'>
                    ...
                    <input id='email' type="email" placeholder='Email ID' name='email' className='border border-gray-200 rounded' value={userDetails?.email ||""} onChange={(e)=>handleEditForm(e,"email")} />
                </div>
                <div className='flex flex-col gap-2'>
                    <label htmlFor="mobile" className='text-gray-500 text-sm'>Mobile</label>
                    <input id='mobile' type="text" maxLength={10} placeholder='Mobile' name='mobile' readOnly className='border border-gray-200 rounded' value={userDetails?.mobile ||""} onChange={(e)=>handleEditForm(e,"mobile")} />
                </div>
                <div className='flex flex-col gap-2'>
                    <label htmlFor="state" className='text-gray-500 text-sm'>State</label>
                    <input id='state' required type="text" placeholder='State' name='state' className='border border-gray-200 rounded' value={userDetails?.state ||""} onChange={(e)=>handleEditForm(e,"state")} />
                </div>
                <div className='flex flex-col gap-2'>
                    <label htmlFor="city" className='text-gray-500 text-sm'>City</label>
                    <input id='city' required type="text" placeholder='City' name='city' className='border border-gray-200 rounded' value={userDetails?.city || ""} onChange={(e)=>handleEditForm(e,"city")} />
                </div>
                <div className='flex flex-col gap-2'>
                    <label htmlFor="address" className='text-gray-500 text-sm'>Locality</label>
                    <input id='address' type="text" placeholder='Address' name='locality' className='border border-gray-200 rounded' value={userDetails?.locality ||""} onChange={(e)=>handleEditForm(e,"locality")} />
                </div>
                <div className='flex flex-col gap-2'>
            ...
            <div className='flex items-center gap-10'>
                <div>
                    ...
                    {doctImage ||userDetails?.doc_file?.length ? (
                        ...
                        <input id='document' type="file" accept="image/*" name='doc_file'  className='hidden' onChange={handleDoctImage} />
                    </div>
                    </div>
                {isAadhar && <div>
                    ...
                    {aadharImage || userDetails?.doc_file_back ? (
                        ...
                        <input id='documentAadhar' type="file" accept="image/*" name='doc_file_back'  className='hidden' onChange={handleAadharImage} />
                    </div>
                </div>}
            </div>
            ...
            <div className='flex flex-col gap-2 my-8'>
                ...
                <textarea placeholder='Describe about yourself/company' name="about" id="about" cols="20" rows="5" className=' p-2 border border-gray-200 rounded' value={userDetails?.about} onChange={(e)=>handleEditForm(e,"about")}></textarea>
            </div>
